...
| Commands / prompts | Markdown slash commands & custom prompts — Claude Code `~/.claude/commands/` (nested subdirectories preserved), Cursor `~/.cursor/commands/`, Codex `~/.codex/prompts/` (deprecated in favor of skills but still supported), OpenCode `~/.config/opencode/commands/` (nested preserved), Qwen Code `~/.qwen/commands/` (nested preserved; deprecated TOML commands are warned, not migrated), Windsurf `~/.codeium/windsurf/global_workflows/` (workflows; flat), Amazon Q CLI `~/.aws/amazonq/prompts/` (saved prompts; flat), CodeBuddy `~/.codebuddy/commands/` (nested preserved; invoked as `/group:command`), Droid `~/.factory/commands/` (nested preserved; shebang script commands are warned, not migrated), Qoder CLI `~/.qoder/commands/` (nested preserved; invoked as `/group:command`), Roo Code `~/.roo/commands/` (flat), Kilo Code `~/.config/kilo/commands/` (flat; legacy `~/.kilocode/workflows/` also read, new location wins), Cline `~/Documents/Cline/Workflows/` (workflows; flat, non-markdown files warned), Auggie CLI `~/.augment/commands/` (nested preserved; invoked as `/namespace:command`), Nanocoder `~/.config/nanocoder/commands/` (nested preserved; directory-as-command bundles export their markdown, `resources/` files warned), Continue `~/.continue/prompts/` (prompt files; nested preserved; `invokable: true` frontmatter required by Continue to list them; legacy v1 `.prompt` files warned, not migrated), VS Code default-profile `User/prompts/*.prompt.md` (Copilot prompt files; flat), Gemini CLI `~/.gemini/commands/**/*.toml` (nested preserved; TOML `prompt`/`description` converted to/from markdown — a format conversion, warned), Crush `~/.config/crush/commands/` (nested preserved; the secondary `~/.crush/commands/` root is also read, XDG wins), Trae `~/.trae/commands/` (nested preserved; the CN-edition `~/.trae-cn/commands/` root is also read, `~/.trae/` wins; Trae reads at most 3 nested levels, deeper imports are warned), Cortex Code `~/.snowflake/cortex/commands/` (nested preserved; user scope only), argument placeholders and frontmatter are client-specific (warned), other clients skip with a warning |
...
- **OpenClaw / Hermes** have no project-scoped files — `--project` covers
  ...
  copilot (`.mcp.json` + `.github/instructions/` + `.github/skills/`), opencode, qwen, amp (`.amp/settings.json` workspace servers), vscode (`.vscode/mcp.json` + `.github/copilot-instructions.md` + `.github/skills/` + `.github/prompts/`), kiro (`.kiro/settings/mcp.json` + `.kiro/steering/` + `.kiro/skills/`), roo (`.roo/mcp.json` + `.roo/rules/` + `.roo/skills/` + `.roo/commands/`), continue (`.continue/mcpServers/` blocks + `.continue/rules/` + `.continue/skills/` + `.continue/prompts/`), crush (`crush.json`/`.crush.json` + `CRUSH.md` + `.crush/skills/` + `.crush/commands/`), antigravity (`.agents/mcp_config.json` + `.agents/rules/` + `.agents/skills/`), droid (`.factory/mcp.json` + `AGENTS.md` + `.factory/skills/` + `.factory/commands/`), amazonq (`.amazonq/mcp.json` + `AmazonQ.md` + `.amazonq/prompts/`), warp (`.warp/.mcp.json` + `AGENTS.md` + `.warp/skills/`), junie (`.junie/mcp/mcp.json` + `.junie/AGENTS.md` + `.junie/skills/`), trae (`.trae/mcp.json` + `.trae/rules/` + `.trae/skills/` + `.trae/commands/`), jetbrains (`.ai/mcp/mcp.json` + `.aiassistant/rules/`), comate (`.comate/mcp.json` + `.comate/rules/` + `.comate/skills/`), codebuddy (`.mcp.json` + `CODEBUDDY.md` + `.codebuddy/skills/` + `.codebuddy/agents/` + `.codebuddy/commands/`), qoder (`.mcp.json` + `AGENTS.md` + `.qoder/skills/` + `.qoder/agents/` + `.qoder/commands/`), auggie (`.augment/settings.json` + `.augment/rules/` + `.augment/skills/` + `.augment/commands/`), kilo (`kilo.json`/`.kilo/kilo.json` + `AGENTS.md` + `.kilo/skills/` + `.kilo/commands/`), kimi (`.kimi-code/mcp.json` + `AGENTS.md` + `.kimi-code/skills/` + `.kimi-code/agents/` + `.agents/agents/`), cortex (`.cortex/mcp.json` + `AGENTS.md` + `.cortex/skills/`), librechat (`librechat.yaml` in the deployment directory), muse (root `AGENTS.md` + `.agents/skills/` + `.agents/memory/`), warp-cli (root `AGENTS.md`/`WARP.md` + `.agents/skills/`), and goose (`.goosehints`/`.goose/memory`/`.agents/skills`;
...
| Kilo Code | `kilo` | `~/.config/kilo/kilo.json` (`mcp` key; `type: local/remote`, native `enabled` flag; kilo.jsonc/config.json also read), `~/.config/kilo/AGENTS.md` (global instructions), `~/.kilo/skills/` (Agent Skills standard), `~/.config/kilo/commands/` (slash commands; legacy `~/.kilocode/workflows/` also read) |
...
| Nanocoder | `nanocoder` | `~/.config/nanocoder/.mcp.json` (`mcpServers` map with explicit `transport`; stdio uses `command`/`args`/`env`, HTTP uses `url`/`headers`, `enabled` flag round-trips; websocket servers skipped), `~/.config/nanocoder/commands/` (custom slash commands; nested namespaces preserved, directory-as-command `resources/` warned); instructions live in the project-root `AGENTS.md` (`--project`), nanocoder skills use their own `skill.yaml` bundle format and are not migrated |
