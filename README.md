...
| Continue | `continue` | `~/.continue/config.yaml` (`mcpServers` list + inline `prompts:`/`rules:`), `~/.continue/mcpServers/` (yaml + json/jsonc MCP block files, incl. claude-code `projects` nesting and single-server files), `~/.continue/rules/` (markdown + yaml block files), `~/.continue/skills/`, `~/.continue/prompts/` (markdown + yaml block files) |
