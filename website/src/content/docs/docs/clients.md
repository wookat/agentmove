...
| Nanocoder | `nanocoder` | `~/.config/nanocoder/.mcp.json` (`mcpServers` map with explicit `transport` (`stdio`/`http`/`websocket`); stdio servers use `command`/`args`/`env`, HTTP servers use `url`/`headers`; the `enabled` boolean round-trips; `timeout`/`alwaysAllow`/`description`/`tags` are client-specific), `~/.config/nanocoder/commands/` (custom slash commands, nested `*.md` preserved as `:`-separated namespaces; a directory containing `<dirname>.md` is a directory-as-command bundle whose markdown is exported and whose `resources/` files are warned, not migrated; project scope adds `.nanocoder/commands/`); instructions live in the project-root `AGENTS.md` only (`--project`); nanocoder skills use their own `skill.yaml` bundle format and are not migrated |
