...
| Claude Code | `claude-code` | `~/.claude.json` (`mcpServers`), `~/.claude/CLAUDE.md`, `~/.claude/skills/`, `~/.claude/agents/` (custom subagents; project scope adds `.claude/agents/`), `~/.claude/commands/` (custom slash commands, subdirectories preserved; project scope adds `.claude/commands/`) |
...
| Codex CLI | `codex` | `~/.codex/config.toml` (`[mcp_servers.*]`, model; `bearer_token_env_var`/`env_http_headers` round-trip as `${VAR}` placeholder headers; `startup_timeout_sec`, `tool_timeout_sec`, `env_vars`, tool approval settings are client-specific and kept on merge), `~/.codex/AGENTS.md`, `~/.agents/skills/`, `~/.codex/prompts/` (custom prompts, invoked as `/prompts:<name>`; deprecated in favor of skills but still supported), `~/.codex/agents/*.toml` (custom agents / agent role files, scanned recursively; project scope `.codex/agents/`; documented conversion: `name` + `description` + `developer_instructions` only — model/reasoning/sandbox/MCP overrides and other settings are dropped with per-field warnings; files missing a required field are warned and not migrated, duplicate role names keep the first file found) |
| Cursor | `cursor` | `~/.cursor/mcp.json`, `~/.cursor/skills/` (global Agent Skills standard; project scope covers `.cursor/skills/`), `~/.cursor/agents/` (custom subagents; project scope adds `.cursor/agents/`; `model`/`read_only`/`is_background` frontmatter is client-specific), `~/.cursor/commands/` (custom slash commands; project scope adds `.cursor/commands/`); instructions/persona imported as `~/.cursor/rules/agentmove-imported.mdc` |
| Gemini CLI | `gemini` | `~/.gemini/settings.json` (`mcpServers`), `~/.gemini/GEMINI.md` (including the "Gemini Added Memories" section), `~/.gemini/skills/` (Agent Skills standard; `~/.agents/skills/` is a native alias), `~/.gemini/agents/` (experimental subagents, enabled by default), `~/.gemini/commands/**/*.toml` (TOML custom commands — `prompt`/`description` converted to/from markdown, nested `:` namespaces preserved); project scope adds `.gemini/skills/` + `.gemini/agents/` + `.gemini/commands/` |
...
| OpenCode | `opencode` | `~/.config/opencode/opencode.json` (`mcp`; local servers use `type: local` with an argv `command` array + `environment`, remote use `type: remote`), `~/.config/opencode/AGENTS.md`, skills from `~/.config/opencode/skills/` plus the singular `skill/`, the `~/.opencode/{skills,skill}/` fallback config dir, and the generic shared root `~/.agents/skills/` (project: `.opencode/{skills,skill}/` + `.agents/skills/`; on duplicate names the first root in that order wins with a warning; imports write only `~/.config/opencode/skills/`), custom agents from `{agent,agents}/` and custom commands from `{command,commands}/` in both config dirs, recursively with nested names preserved (the `~/.opencode/` fallback dir wins duplicates with a warning, matching opencode's last-config-dir-wins merge; project scope reads `.opencode/{agents,agent}/` and `.opencode/{commands,command}/`; imports write only `~/.config/opencode/agents/` and `commands/`, project `.opencode/agents/` and `commands/`), primary modes from `{mode,modes}/*.md` in both config dirs (flat scan; a mode beats a same-name agent within its config dir, matching opencode's merge order; exported into the agents layer with a per-entry warning — imports never synthesize mode files), inline `agent`/`command`/`mode` entries in `opencode.json`/`opencode.jsonc` of both config dirs (exported as synthesized markdown with per-entry warnings; the inline `mode` map merges last and wins duplicates, markdown files beat inline `agent`/`command` entries from the same config dir; project scope also reads the root and `.opencode/` `opencode.json(c)` files; imports never write inline config entries) |
...
| Amp | `amp` | `~/.config/amp/settings.json` (`amp.mcpServers`; local servers use `command`/`args`/`env`, remote use `url`/`headers`), `~/.config/amp/AGENTS.md`, `~/.config/agents/skills/` + `~/.agents/skills/` + `~/.config/amp/skills/` |
| VS Code | `vscode` | user-profile `mcp.json` (`servers`; stdio entries use `command`/`args`/`env`, remote use `type: http`/`sse` + `url`/`headers`); profile folder is `~/.config/Code/User` (Linux), `~/Library/Application Support/Code/User` (macOS), or `%APPDATA%\Code\User` (Windows) — all three are checked; `~/.agents/skills/` (personal Agent Skills, shared cross-agent root); default-profile `User/prompts/*.prompt.md` (Copilot prompt files as commands; flat, other customization types in that folder stay with their own layers; project scope adds `.github/prompts/*.prompt.md`) |
| Kiro | `kiro` | `~/.kiro/settings/mcp.json` (`mcpServers`; stdio entries use `command`/`args`/`env` + native `disabled`, remote use `url`/`headers`), steering markdown in `~/.kiro/steering/` (AGENTS.md standard supported), `~/.kiro/skills/` (Agent Skills standard), `~/.kiro/agents/*.md` (custom agents; project scope adds `.kiro/agents/`; JSON-format agent configs are not migrated — warned; `tools`/`model`/`permissions` frontmatter is client-specific), `~/.kiro/prompts/*.md` (saved prompts as commands; flat, invoked as `@name`, no arguments; project scope adds `.kiro/prompts/` — workspace prompts override global ones with the same name) |
| Roo Code | `roo` | VS Code globalStorage `mcp_settings.json` (`mcpServers`; stdio entries use `command`/`args`/`env` + native `disabled`, remote entries require an explicit `type: streamable-http`/`sse` + `url`/`headers`) — `~/.config/Code/User` (Linux), `~/Library/Application Support/Code/User` (macOS), or `%APPDATA%\Code\User` (Windows); rules markdown in `~/.roo/rules/`, `~/.roo/skills/` (Agent Skills standard), `~/.roo/commands/*.md` (custom slash commands, flat; project scope adds `.roo/commands/`); custom modes in `settings/custom_modes.yaml` next to `mcp_settings.json` (documented conversion: `slug` + `description` + `roleDefinition`, other fields warned; project scope: `.roomodes`) |
| Continue | `continue` | `~/.continue/config.yaml` (`mcpServers` **list**; each entry carries its own `name`, stdio uses `command`/`args`/`env`/`cwd`, remote uses `type: streamable-http`/`sse` + `url`) plus `~/.continue/mcpServers/` block files (discovered recursively in nested subdirectories, like all Continue block dirs incl. `prompts`/`rules` YAML blocks; YAML `mcpServers:` lists and JSON/JSONC files in every format Continue's loader accepts — claude-style maps, claude-code `projects` nesting, single-server files named after the file; config.yaml wins duplicate names with a warning; imports write config.yaml only; project scope reads `.continue/mcpServers/`), rules markdown in `~/.continue/rules/` plus inline `rules:` entries in `config.yaml` and `.continue/rules/*.yaml` block files (merged into the instructions document; scoping metadata warned and dropped), `~/.continue/skills/`, `~/.continue/prompts/` (prompt files as slash commands; nested `*.md` preserved, `invokable: true` frontmatter required by Continue to list them, legacy v1 `.prompt` files warned and not migrated; inline `prompts:` blocks in `config.yaml` and `.continue/prompts/*.yaml` block files exported as synthesized markdown with `invokable: true`, markdown files winning duplicates; imports write only prompt files; project scope adds `.continue/prompts/`) |
...
| Antigravity | `antigravity` | `~/.gemini/config/mcp_config.json` (`mcpServers`; stdio uses `command`/`args`/`env`/`cwd`, remote servers require `serverUrl` — legacy `url`/`httpUrl` are not supported — plus `headers`, and a native `disabled` flag), `~/.gemini/config/skills/` (Agent Skills standard), `~/.gemini/config/global_workflows/*.md` (workflows as commands; flat, triggered as `/name` in AGY and AGY IDE — AGY CLI lists workflows but cannot trigger them; project scope: `.agents/workflows/`); global rules live in `~/.gemini/GEMINI.md`, shared with Gemini CLI. This shared config is used by all Antigravity 2.0 surfaces — the desktop app, the IDE, and the Antigravity CLI (`agy`) — so one migration covers all three |
...
| Trae | `trae` | `~/.trae/skills/` (global Agent Skills standard), `~/.trae/commands/` global commands (nested; the CN-edition `~/.trae-cn/commands/` root is also read, `~/.trae/` wins on conflicts; imports write only `~/.trae/commands/`); user-level MCP servers, rules, and memories are app-managed (Settings UI) — project scope covers `.trae/mcp.json` (`mcpServers`; entries have no `type` field — stdio uses `command`/`args`/`env`, remote uses `url`/`headers`), `.trae/rules/*.md`, `.trae/skills/`, and `.trae/commands/` (up to 3 nested levels) |
| CodeBuddy | `codebuddy` | `~/.codebuddy/.mcp.json` (`mcpServers`; `type` is `stdio`/`sse`/`http` and may be omitted — stdio uses `command`/`args`/`env`, remote uses `url`/`headers`; a top-level `disabledMcpServers` name list carries the disabled state; JSONC accepted; `~/.codebuddy/mcp.json` and `~/.codebuddy.json` are read as legacy fallbacks), `~/.codebuddy/CODEBUDDY.md` (user memory file), `~/.codebuddy/skills/` (Agent Skills standard), `~/.codebuddy/agents/*.md` (custom sub-agents; project scope adds `.codebuddy/agents/`; `tools`/`model`/`effort`/`maxTurns`/`memory`/`mcpServers` frontmatter is client-specific), `~/.codebuddy/commands/` (custom slash commands, nested `*.md` preserved as `/group:command` names; project scope adds `.codebuddy/commands/`) |
| Qoder CLI | `qoder` | `~/.qoder/settings.json` (`mcpServers` key inside the general settings file — other settings are preserved on rewrite; `type` is `stdio`/`sse`/`http`/`ws` and may be omitted for stdio — stdio uses `command`/`args`/`env`, remote uses `url`/`headers`), `~/.qoder/AGENTS.md` (user memory file), `~/.qoder/skills/` (Agent Skills standard), `~/.qoder/agents/*.md` (custom subagents; project scope adds `.qoder/agents/`; `tools`/`model`/`skills`/`mcpServers` frontmatter is client-specific), `~/.qoder/commands/` (custom slash commands, nested `*.md` preserved as `/group:command` names; project scope adds `.qoder/commands/`) |
| Auggie CLI | `auggie` | `~/.augment/settings.json` (`mcpServers` key inside the general settings file — other settings are preserved on rewrite; `type` is `stdio`/`sse`/`http` and may be omitted for stdio — stdio uses `command`/`args`/`env`, remote uses `url`/`headers`), `~/.augment/rules/*.md` (user rules, always applied), `~/.augment/skills/` (Agent Skills standard), `~/.augment/commands/` (custom slash commands, nested `*.md` preserved as `/namespace:command` names; project scope adds `.augment/commands/`; the compatibility roots `~/.claude/commands/` and `~/.agents/commands/` belong to other adapters and are not read or written), `~/.augment/agents/` (custom subagents, recursive `*.md`/`*.txt` with nested names preserved — `.txt` agents are exported with a warning and imports always write `.md`; project scope adds `.augment/agents/`; `name`/`description`/`color`/`model`/`tools`/`disabled_tools` frontmatter is client-specific; the compatibility roots `~/.claude/agents/` and `~/.agents/agents/` belong to other adapters and are not read or written) |
| Kilo Code | `kilo` | `~/.config/kilo/kilo.json` (`mcp` key inside the general config file — other keys are preserved on rewrite; `kilo.jsonc`/`config.json` also read, JSONC accepted; local servers use `type: "local"` with `command` as an argv array plus `environment`, remote servers use `type: "remote"` + `url`/`headers`; native `enabled` flag round-trips), `~/.config/kilo/AGENTS.md` (global instructions), `~/.kilo/skills/` (Agent Skills standard; shared by the CLI and the VS Code/JetBrains extensions), `~/.config/kilo/agents/` + `~/.config/kilo/agent/` (custom agents / custom modes, recursive `*.md` with nested names preserved; legacy `~/.kilocode/` and `~/.kilo/` roots also read with `~/.config/kilo/` winning on name conflicts, imports write only `~/.config/kilo/agents/`; project scope: `.kilo/agents/`/`.kilo/agent/` + legacy `.kilocode/`), `~/.config/kilo/commands/` (slash commands, flat `*.md`; legacy `~/.kilocode/workflows/` still read with the new location winning on name conflicts, imports write only the new location; project scope: `.kilo/commands/` + legacy `.kilocode/workflows/`) |
| Kimi Code CLI | `kimi` | `~/.kimi-code/mcp.json` (`mcpServers` key; stdio uses `command`/`args`/`env`/`cwd`, HTTP uses a plain `url` with optional `headers`, legacy SSE sets `transport: "sse"`; native `enabled` flag round-trips; `bearerTokenEnvVar`/`startupTimeoutMs`/`toolTimeoutMs`/`enabledTools`/`disabledTools` are client-specific), `~/.kimi-code/AGENTS.md` (global instructions), `~/.kimi-code/skills/` plus the generic shared root `~/.agents/skills/` (Agent Skills standard; on duplicate names — compared case-insensitively — the `.kimi-code/skills/` copy wins with a warning, matching kimi's brand-then-generic scanner priority; imports write only `~/.kimi-code/skills/`; `$KIMI_CODE_HOME` relocations are not followed), `~/.kimi-code/agents/` + shared `~/.agents/agents/` (custom agents, scanned recursively; project scope adds `.kimi-code/agents/` + `.agents/agents/`; imports write only `.kimi-code/agents/`; `tools`/`disallowedTools`/`subagents`/`model_preference`/`override` frontmatter is client-specific) |
| Grok CLI | `grok` | `~/.grok/config.toml` (`[mcp_servers.*]` tables inside the general config file — other tables are preserved on rewrite; stdio servers use `command`/`args`/`env`, remote servers use `url`/`headers`; `startup_timeout_sec`/`tool_timeout_sec` are client-specific; `${VAR}` placeholders expand natively at load time), `~/.grok/AGENTS.md` (global rules), `~/.agents/skills/` (Agent Skills standard — grok's real skills root; the legacy `~/.grok/skills/` written by older agentmove versions is still read with a warning, imports write only `~/.agents/skills/`), `subAgents` entries in `~/.grok/user-settings.json` (custom foreground sub-agents `{name, model, instruction}`, invoked by exact name via the task tool; documented conversion: `instruction` only — the per-agent `model` is dropped with a warning on export and imports assign grok's default model; reserved built-in names, duplicates, and instruction-less entries are warned and not migrated; other user-settings keys are preserved on rewrite) |
| Vibe Code CLI | `vibe` | `~/.vibe/config.toml` (`[[mcp_servers]]` array of tables with explicit `transport` (`stdio`/`http`/`streamable-http`) inside the general config file — other keys are preserved on rewrite; stdio servers use `command`/`args`/`env`, remote servers use `url`/`headers`; `api_key_env`/`api_key_header`/`api_key_format`, `startup_timeout_sec`/`tool_timeout_sec`, and `enabled_tools`/`disabled_tools` are client-specific), `~/.vibe/AGENTS.md` (global instructions), `~/.vibe/skills/` (Agent Skills standard), `~/.vibe/agents/` (custom agents: flat TOML config-override profiles; a profile's `system_prompt_id` prompt in `~/.vibe/prompts/` exports as the agent body, other overrides are client-specific) |
| Nanocoder | `nanocoder` | `~/.config/nanocoder/.mcp.json` (`mcpServers` map with explicit `transport` (`stdio`/`http`/`websocket`); stdio servers use `command`/`args`/`env`, HTTP servers use `url`/`headers`; the `enabled` boolean round-trips; `timeout`/`alwaysAllow`/`description`/`tags` are client-specific), `~/.config/nanocoder/commands/` (custom slash commands, nested `*.md` preserved as `:`-separated namespaces; a directory containing `<dirname>.md` is a directory-as-command bundle whose markdown is exported and whose `resources/` files are warned, not migrated; project scope adds `.nanocoder/commands/`), `~/.config/nanocoder/agents/` (custom subagents; flat `*.md`, copied as-is — imports inject the required `name`/`description` frontmatter when missing, warned; project scope adds `.nanocoder/agents/`), `~/.config/nanocoder/skills/` (skill bundles; each `<bundle>/skill.yaml` directory's `commands/*.md` export as `<bundle>/<name>` commands and its single `agents/*.md` subagent exports by file name — bundle `tools/` (nanocoder shell tools) and `skill.yaml` extras (`version`/`author`/`tags`/`subscribe`/`tools_visibility`) are warned, not migrated; flat-directory names win on collision; project scope adds `.nanocoder/skills/`); instructions live in the project-root `AGENTS.md` only (`--project`); Agent Skills-standard skills are not migrated |
...
| Cortex Code | `cortex` | `~/.snowflake/cortex/mcp.json` (`mcpServers` key with an explicit `type: stdio/http/sse` on every entry — stdio uses `command`/`args`/`env`/`cwd`, remote uses `url`/`headers`; the per-server `timeout` is client-specific; no per-server disabled flag), `~/.snowflake/cortex/AGENTS.md` (user-scope instruction file, the same file the CoCo Personalization editor writes), `~/.snowflake/cortex/skills/` (Agent Skills standard), `~/.snowflake/cortex/commands/` (custom commands, nested preserved; user scope only — no project commands directory is documented); agent-managed memory under `~/.snowflake/cortex/memory/` is not migrated; project scope covers `.cortex/mcp.json`, root `AGENTS.md`, and `.cortex/skills/` |
...
- **Cline** VS Code extension keeps its own MCP settings copy in VS Code
  ...
  (project `.cline/skills/` with `--project`). Workflows (commands) are
  flat — nested bundle names are flattened (`git/commit` → `git-commit`,
  warned) and non-markdown workflow files are not migrated (warned per
  file); enable/disable toggles are app-managed and not migrated.
...
- **CodeBuddy** disabled state round-trips via the top-level
  ...
  Code project scope uses), `CODEBUDDY.md`, `.codebuddy/skills/`, and
  `.codebuddy/agents/`.
...
- **Kilo Code** has no `sse` type — SSE servers are emitted as `remote`
  ...
  (warned). Commands are flat — nested bundle names are flattened
  (`git/commit` → `git-commit`, warned); `description`/`agent`/`model`/
  `subtask` frontmatter and argument placeholders are client-specific
  (warned); legacy `~/.kilocode/workflows/` files are still exported
  (warned) but imports write only `~/.config/kilo/commands/`. `--project`
  covers `kilo.json`/`.kilo/kilo.json(c)`, root `AGENTS.md`,
  `.kilo/skills/`, and `.kilo/commands/`.
- **Kimi Code CLI** per-server `bearerTokenEnvVar`, `startupTimeoutMs`,
  ...
  `.kimi-code/skills/` plus `.agents/skills/` (brand root wins duplicates;
  imports write only `.kimi-code/skills/`).
...
- **Grok CLI** has no documented `sse` transport — SSE servers are emitted as
  ...
  covers `.grok/config.toml`, root `AGENTS.md`, and `.agents/skills/` (legacy
  `.grok/skills/` still read with a warning); custom sub-agents are
  user-level only.
...
- **Nanocoder** `websocket` servers have no portable equivalent and are
  ...
  `AGENTS.md` from the project root only (use `--project`); Agent
  Skills-standard skills are skipped (warned) — nanocoder's own skill
  bundles under `~/.config/nanocoder/skills/` export their portable members
  instead (commands as `<bundle>/<name>`, the single subagent by file name;
  bundle `tools/` and manifest extras warned); memory has no durable store —
  skipped (warned). `--project` covers `.mcp.json`, the root `AGENTS.md`,
  and `.nanocoder/skills/`.
