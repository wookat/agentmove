...
async function importTo(
  ...
  const supportsAgents = project
    ? (getProjectAdapter(adapter.id).supportsAgents ?? false)
...
program
  ...
  .action(
    async (
      ...
      } else if (await isSkillsRepo(opts.in)) {
        const repo = await readSkillsRepo(opts.in);
        bundle = repo.bundle;
        mifWarnings.push(...repo.warnings);
      } else {
